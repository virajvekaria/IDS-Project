...
class SearchService:
    ...
    def search(
        ...
        Returns:
            Tuple of (search_results, answer)
        """
        # Get index store
        ...
        for result in search_results:
            ...
            document = db.query(Document).filter(Document.filename == result["source_doc"]).first()
            if document:
    ...
    def chat(
        ...
        Returns:
            Tuple of (conversation_id, answer, references)
        """
        ...
        if conversation_id:
            ...
            if not conversation:
                ...
                conversation = ConversationService.create_conversation(db, schemas.ConversationCreate())
        ...
        for doc in retrieved_docs:
            # Get document ID from chunk
            document = db.query(Document).filter(Document.filename == doc["source_doc"]).first()
            if document:
                # Get chunk ID
