...
def get_document_chunks(
    document_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    """
...
def get_document_jobs(
    ...
    jobs = DocumentService.get_document_jobs(
        db, document_id=document_id, skip=skip, limit=limit
    )
    return jobs
...
def process_document_background(
    ...
    DocumentService.process_document(
        db, document_id, chunk_size, chunk_overlap, index_name
    )
...
def process_document(
    ...
    extraction_job = ProcessingJob(
        document_id=request.document_id, job_type="extraction", status="pending"
    )
    chunking_job = ProcessingJob(
        document_id=request.document_id, job_type="chunking", status="pending"
    )
    indexing_job = ProcessingJob(
        document_id=request.document_id, job_type="indexing", status="pending"
    )
    ...
    return schemas.ProcessDocumentResponse(
        document_id=request.document_id, status="processing", job_ids=job_ids
    )
